...
def run_query(model: str, query: str):
    ...
    client_script = os.path.join(base_root, "src", "generator", "lance_search_client.py")
    python_bin = os.path.join(base_root, ".venv", "bin", "python3")
    
    start_t = time.time()
    try:
        res = subprocess.run([python_bin, client_script, "--model", model, "--query", query],
    ...
    for q in queries:
        ...
        for m in models:
            ...
            q_res["results"][m] = {
                ...
                "top_distance": round(top_chunk["_distance"], 3) if top_chunk and "_distance" in top_chunk else None,
                "top_repo": top_chunk["repo_name"] if top_chunk else None,
                "top_file": top_chunk["file_path"] if top_chunk else None,
                "snippet": top_chunk["text"][:200].replace("\n", " ") if top_chunk else "No results"
            }
            print(f" - {m} -> {len(chunks)} chunks in {dur:.2f}s (Top dist: {q_res['results'][m]['top_distance']})")
