...
if (!fs.existsSync(runDir) && fs.existsSync(baseDataDir)) {
  console.error("Bootstrapping isolated runtime DB workspace from tracked base structures...");
  try {
...
server.onRequest(ListPromptsRequestSchema, async () => {
  return {
    prompts: [{
      ...
      description: "System instructions for processing cross-repository updates and feature modules."
    }]
  };
});
...
server.onRequest(GetPromptRequestSchema, async (request) => {
  ...
  const content = fs.existsSync(skillsPath) ? fs.readFileSync(skillsPath, "utf-8") : "Skill documentation not located.";
  return {
    description: "System guidelines governing codebase updates and cross-repo features.",
    messages: [{
      role: "user",
...
server.onRequest(ListToolsRequestSchema, async () => {
  return {
    tools: [{
      ...
      description: "Searches local persistent multi-repository distinct context tables in parallel delegating semantic matching engine.",
      inputSchema: {
        type: "object",
        properties: {
          query_text: {
            type: "string",
            description: "Natural language text description of target features, protocols, or multi-repo components."
          },
          model: {
            type: "string",
            enum: ["gemma", "nomic"],
            description: "Embedding model target layer to search against. nomic leverages an expansive code-syntax optimized context."
          }
        },
        required: ["query_text"]
      }
...
server.onRequest(CallToolRequestSchema, async (request) => {
  ...
  try {
    ...
    if (!fs.existsSync(sidecarPath) || !fs.existsSync(pythonInterpreter)) {
      return { content: [{ type: "text", text: "Universal searching sidecar logic script or Python interpreter not found." }] };
    }
    ...
    if (parsed.error) {
      return { content: [{ type: "text", text: `Database lookup trace reported: ${parsed.error}` }] };
    }
    
    const results = parsed.results || [];
    if (results.length === 0) {
      return { content: [{ type: "text", text: "No relevant matching context blocks retrieved across distinct repository tables." }] };
    }
    
    const formattedText = results.map(r => {
      const repo = r.repo_name ? `[Repo: ${r.repo_name}]` : `[Table: ${r.table}]`;
      const file = r.file_path ? `[File: ${r.file_path}]` : "";
      const dist = r._distance !== undefined ? ` [Distance: ${Number(r._distance).toFixed(3)}]` : "";
...
console.error("Universal Context Protocol (ucp-mcp) Delegated Server connected successfully.");
