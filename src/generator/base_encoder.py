...
class BaseEmbeddingGenerator(ABC):
    def __init__(self, model_name: str, dimensions: int):
        ...
        self.config_path = os.path.abspath(os.path.join(os.path.dirname(__file__), "sources.json"))
    ...
    def generate_vectors(self, texts: list[str], is_query: bool = False) -> list[list[float]]:
    ...
    def _clean_html(self, html_str: str) -> str:
        ...
        if not main_match:
            main_match = re.search(r'<article[^>]*>(.*?)</article>', html_str, re.DOTALL | re.IGNORECASE)
        if not main_match:
            main_match = re.search(r'<div[^>]*class="[^"]*md-content[^"]*"[^>]*>(.*)', html_str, re.DOTALL | re.IGNORECASE)
    ...
    def _clean_code_content(self, content: str, file_path: str) -> str:
        ...
        if content_stripped.startswith("/*"):
            ...
            if end_idx != -1:
                ...
                if any(kw in header.lower() for kw in ["copyright", "license", "apache", "mit", "warranty", "is distributed on an"]):
        ...
        for line in lines:
            ...
            if sline.startswith("#") or sline.startswith("//"):
                ...
                if any(kw in line_lower for kw in ["copyright", "license", "apache", "mit", "warranty", "basis,", "is distributed on an"]):
                    has_license_kw = True
            else:
                break
    ...
    def shallow_clone(self, repo_config: dict) -> str:
        ...
        try:
            subprocess.run(["git", "clone", "--depth", "1", "--branch", branch, url, target_dir], check=True)
        except subprocess.CalledProcessError as e:
            print(f"Error cloning repository {name}: {e}")
        return target_dir

    def get_files_to_index(self, base_path: str, includes: list[str], excludes: list[str]) -> list[str]:
        matched_files = set()
        cwd = os.getcwd()
        try:
            os.chdir(base_path)
    ...
    def run(self):
        ...
        print(f"Starting highly modular embedding pipeline for model tier: {self.model_name}")
        ...
        for repo in config.get("repositories", []):
            ...
            if total_chunks == 0:
                print(f"No valid extracted text chunks found for repository '{name}'. Skipping embedding generation.", flush=True)
                continue
                
            print(f"Extracted {total_chunks} semantic chunks from repository '{name}'. Initiating vectorized bulk embedding phase...", flush=True)
            ...
            for b_start in range(0, total_chunks, batch_size):
                ...
                try:
                    ...
                    print(f"Error processing vector array chunk block [{b_start+1}:{b_start+len(b_chunks)}]: {e}. Supplying fallback isolation arrays.", flush=True)
                    b_vectors = [[0.01] * self.dimensions for _ in b_texts]
                    
                b_duration = time.time() - b_start_time
                print(f"Processed vector slice [{b_start+1} - {b_start+len(b_chunks)} / {total_chunks}] in {b_duration:.2f}s", flush=True)
        ...
        for repo_name, group_df in df_all.groupby("repo_name"):
            base_table_name = "".join([c if c.isalnum() or c in "-_" else "_" for c in repo_name])
            ...
            if total_repo_rows <= max_rows_per_table:
                print(f"Committing distinct isolated table partition: {base_table_name}...", flush=True)
                table = db.create_table(base_table_name, data=group_df, mode="overwrite")
                try:
                    table.optimize()
                except Exception:
                    pass
            else:
                print(f"Repository '{repo_name}' contains {total_repo_rows} rows. Partitioning into multi-table fragments to enforce <100MB Git push limits...", flush=True)
                for p_idx, start_r in enumerate(range(0, total_repo_rows, max_rows_per_table)):
                    sub_df = group_df.iloc[start_r:start_r + max_rows_per_table]
                    sub_table_name = f"{base_table_name}_part{p_idx+1}"
                    print(f" - Committing fragmented sub-table: {sub_table_name} ({len(sub_df)} rows)...", flush=True)
                    table = db.create_table(sub_table_name, data=sub_df, mode="overwrite")
                    try:
                        table.optimize()
                    except Exception:
                        pass
        ...
        print(f"Pipeline processing complete! Populated {len(df_all['repo_name'].unique())} distinct repository tables under model layer '{self.model_name}' in {duration:.2f} seconds.")
